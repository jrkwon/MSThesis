...
unsigned int nKStable[2350] = {        /* KS 완성형 2350자의 조합형 코드 */
 ...
 *    종성 : ㄱㄲㄳㄴㄵㄶㄷㄹㄺㄻㄼㄽㄿㄾㅀㅁㅂㅄㅅㅆㅇㅈㅊㅋㅌㅍㅎ
 */

/*
 *    조합형 자소를 완성형의 자소로 바꾸려면 초성, 중성, 종성 모두
 *    0xA4A0 + index + offset
 */

#define  KS_SEG   0xA4A0   /* 완성형 한글 낱자의 시작 코드 */

/*
 *    초성 자소의 offset = 0
 */

static unsigned int nFirst[] = {
  0x8841, 0x8C41, 0x9041, 0x9441, 0x9841, 0x9C41, 0xA041, 0xA441, 0xA841,
  0xAC41, 0xB041, 0xB441, 0xB841, 0xBC41, 0xC041, 0xC441, 0xC841, 0xCC41,
  0xD041, 0x0000
};
...
/*
 *    완성형으로 변환하기 위한 초성 색인표
 */

static int nIndex1st[] = {
       1,      2,      4,      7,      8,      9,     17,     18,     19,
 ...
 *   중성 자소의 offset = 31
 */
static unsigned int nMid[] = {
  0x8461, 0x8481, 0x84A1, 0x84C1, 0x84E1, 0x8541, 0x8561, 0x8581, 0x85A1,
  0x85C1, 0x85E1, 0x8641, 0x8661, 0x8681, 0x86A1, 0x86C1, 0x86E1, 0x8741,
  0x8761, 0x8781, 0x87A1, 0x0000
};

static int nIndexMid[] = {
       0,      1,      2,      3,      4,      5,      6,      7,      8,
...
static unsigned int nLast[] = {
  0x8442, 0x8443, 0x8444, 0x8445, 0x8446, 0x8447, 0x8448, 0x8449, 0x844A,
  0x844B, 0x844C, 0x844D, 0x844E, 0x844F, 0x8450, 0x8451, 0x8453, 0x8454,
...
/*
 *    완성형으로 변환하기 위한 중성 색인표
 */

static int nIndexLast[] = {
       1,      2,      3,      4,      5,      6,      7,      9,     10,
...
/*
 *  상용조합형 낱자를 완성형으로 변환하기 위해 이용하는 표
 */
static int nOffsetTable[] = { 0, 31, 0 };
static int nTGoffset[] = { 0, 30, 0 };

unsigned nTGcharEle[][32] = {
  /* 초성 TG 코드 */
  { 0x8841, 0x8C41, 0x0000, 0x9041, 0x0000, 0x0000, 0x9441, 0x9841, 0x9C41,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0xA041, 0xA441,
    0xA841, 0x0000, 0xAC41, 0xB041, 0xB441, 0xB841, 0xBC41, 0xC041, 0xC441,
    0xC841, 0xCC41, 0xD041, 0x0000 },

/*
 *    ㄱ      ㄲ     NULL     ㄴ     NULL    NULL     ㄷ      ㄸ      ㄹ
 *   NULL    NULL    NULL    NULL    NULL    NULL    NULL     ㅁ      ㅂ
 *    ㅃ     NULL     ㅅ      ㅆ      ㅇ      ㅈ      ㅉ      ㅊ      ㅋ
 *    ㅌ      ㅍ      ㅎ
 */
  ...
  /* 종성 TG 코드 */
  { 0x8442, 0x8443, 0x8444, 0x8445, 0x8446, 0x8447, 0x8448, 0x0000, 0x8449,
    0x844A, 0x844B, 0x844C, 0x844D, 0x844E, 0x844F, 0x8450, 0x8451, 0x8453,
    0x0000, 0x8454, 0x8455, 0x8456, 0x8457, 0x8458, 0x0000, 0x8459, 0x845A,
    0x845B, 0x845C, 0x845D, 0x0000 }
/*
 *    ㄱ      ㄲ      ㄳ      ㄴ      ㄵ      ㄶ      ㄷ     NULL     ㄹ
 *    ㄺ      ㄻ      ㄼ      ㄽ      ㄿ      ㄾ      ㅀ      ㅁ      ㅂ
 *   NULL     ㅄ      ㅅ      ㅆ      ㅇ      ㅈ      NULL    ㅊ      ㅋ
 *    ㅌ      ㅍ      ㅎ
 */
 ...
 *  설  명 :  완성형 한글 낱자 인지
 */
int IsKSsingle(nKScode)
unsigned nKScode;
{
  return ((nKScode > 0xA4A0) && (nKScode < 0xA4D4)) ? true : false;
}

/*
 *  입력   :  비교할 문자열 포인터(p) 비교할 문자열 포인터(q)
 *  출력   :  같으면 0, *pnPtr1 - *pnPtr2
 *  호출함 :
 *  호출됨 :
 *  설  명 :  bsearch함수에서  사용할 비교 함수
 */
static int CodeCmp(pnPtr1, pnPtr2)
unsigned int *pnPtr1;
unsigned int *pnPtr2;
{
   return (*pnPtr1 - *pnPtr2);
}

/*
 *  입력   :  상용조합형 한글 한글자의 코드값, 글자의 종류(완전한 글자, 초, 중, 종성 중 하나)
 *  출력   :  KS완성형 한글 코드, 변환실패면 0
 *  호출함 :
 *  호출됨 :
 *  설  명 :  상용조합형코드의 한글 한글자를 KS완성형 한글 한글자로 바꾼다,
 *            한글 낱자가 들어올 때면 언제나 초성, 중성, 종성 중 어느 하나만이
 *            들어와야 한다.
 */
unsigned int TG2KS(nTGcode, nKind)
unsigned int nTGcode;
int nKind;
{
  int nCnt, *pnIndex, nOffset, nIndex;
  unsigned int *pnCode, *pnTemp;

  /* 완전한 글자의 코드 변환 */
  if(nKind == FULL_CODE) {
    if((pnTemp = (unsigned  int *)bsearch(&nTGcode, nKStable,
                                sizeof(nKStable) / sizeof(unsigned int),
                                sizeof(unsigned int), CodeCmp)) != NULL) {
      nIndex = pnTemp - nKStable;
      return (((nIndex / 94) + 0xb0) << 8) + (nIndex % 94) + 0xa1;
    }
    else { /* 그런 글자 없다 */
      return 0;
    }
  ...
  /* 한글 낱자의 코드 변환 */
  else if(nKind == FIRST_CODE) {
    pnCode = nFirst, pnIndex = nIndex1st;
  }
  else if(nKind == MID_CODE) {
    pnCode = nMid, pnIndex = nIndexMid;
  }
  else if(nKind == LAST_CODE) {
    pnCode = nLast, pnIndex = nIndexLast;
  }
  else { /* 잘못 들어왔다... */
    return 0;
  }
  nOffset = nOffsetTable[nKind];
  for(nCnt = 0; pnCode[nCnt]; nCnt++) {
    if(nTGcode == pnCode[nCnt]) {
       return KS_SEG + pnIndex[nCnt] + nOffset;
    } /* if */
    else if(nTGcode < pnCode[nCnt])
      break;
  } /* for */
  return 0; /* 이까지 오면 안된다. */
}
...
/*
 *  입력   :  KS완성형 한글 한글자의 코드값
 *  출력   :  상용조합형 한글 한글자의 코드, 변환실패면 0
 *  호출함 :
 *  호출됨 :
 *  설  명 :  완성형 한글 한글자를 상용조합형 한글 한글자로 변환한다.
 */
unsigned int KS2TG(nKScode)
unsigned int nKScode;
{
  int   nIndex;
  int   nChar1, nChar2;

  nChar1 = nKScode >> 8;
  nChar2 = nKScode & 0x00ff;
  if (nChar1 < 0xb0 || nChar1 > 0xc8 || nChar2 < 0xa1 || nChar2 > 0xfe)
    return 0;
  nIndex = (nChar1 - 0xb0) * 94 + nChar2 - 0xa1;
  return nKStable[nIndex];
}

/*
 *  입력   :  KS완성형 한글 한 자소의 코드값
 *  출력   :  상용조합형 한글 받침만 있는 글자의 코드, 변환실패면 0
 *  호출함 :
 *  호출됨 :
 *  설  명 :  완성형 한글 자소 하나를 상용조합형 한글 받침만 있는 글자로 변환한다.
 */
unsigned int KS2ElementTG(nKScode, nKind)
unsigned int nKScode;
int nKind;
{
  int nIndex;

  if (!IsKSsingle(nKScode))
    return 0;

  nIndex = nKScode - (KS_SEG + 1 + nTGoffset[nKind]);
  return nTGcharEle[nKind][nIndex];
}

/*
 *  완성형 한글 한글자를 자소 분리하는 알고리즘, 그 첫째판
 *
 *  예 : 원하시는
 *
 *  1. 함수는 다음과 같은 꼴을 갖춘다.
 *
 *     전달되는 한글코드도 완성형, 전달 받는 코드도 완성형
 *
 *     SplitHangulCharElement(unsigned nKScode, unsigned *pnKScode1, unsigned *pnKScode2);
 *
 *     nKScode = 는,   pnKScode1 = 느,   pnKScode2 = ㄴ
 *
 *  2. 분리할 글자가 완성형 코드에서 한글 자소(0xA4A1 ~ 0xA4D3)에 해당한다면
 *     아예, 이 함수를 부르지도 말아라.
 *
 *     원하시ㄴ +  ㅡ,ㄴ
 *
 *     이 경우, ㄴ이 한글 자소코드에 해당하므로, SplitHangulCharElement를 부르지 않고,
 *
 *     원하시  +  는
 *
 *     의 꼴로 만들어 준다.
 ...
 */
/*
 *  입력   :  KS완성형 한글 한글자의 코드값, 분리한 코드를 담을 두 변수
 *  출력   :
 *  호출함 :
 *  호출됨 :
 *  설  명 :  완성형 한글 한글자를 두개로 분리한다. 자세한 것은 위의 알고리즘 참고.
 */
void SplitHangulCharElement(nKScode, pnKScode1, pnKScode2)
unsigned int nKScode;
unsigned int *pnKScode1;
unsigned int *pnKScode2;
{
  int n1st, n2nd, n3rd;
  unsigned nTGcode, nTGcode1, nTGcode2;

  /*
   *  완성형에서 ---> 조합형으로 코드변환
   *  이때, 초성만 있는 완성형 자소 코드는 들어오면 안되지.
   */
  nTGcode = KS2TG(nKScode);

  /* nTGcode를 nTGcode1와 nTGcode2로 분리한다. */

  n1st = (nTGcode >> 10) & 0x1F;   /* 초성 분리 */
  n2nd = (nTGcode >> 5) & 0x1F;    /* 중성 분리 */
  n3rd = (nTGcode & 0x1F);         /* 종성 분리 */

  /* 종성이 있다면 */
  if(n3rd != FILL_LST) {
    /* 초성과 중성으로 한 글자, 종성만으로 한 글자를 완성 */
    nTGcode1 = 0x8000 + (n1st << 10) + (n2nd << 5) + FILL_LST;
    nTGcode2 = 0x8000 + (FILL_1ST << 10) + (FILL_MID << 5) + n3rd;

    /* TG2KS는 조합형 자소도 완성형 자소로 바꾸어주어야한다. */
    *pnKScode1 = TG2KS(nTGcode1, FULL_CODE);
    *pnKScode2 = TG2KS(nTGcode2, LAST_CODE);
    return;
  }
  /* 종성은 없고 초성과 중성만으로 이루어진 글자라면 */
  if(n2nd != FILL_MID) {
    /* 초성을 하나로 떼어내고, 중성을 하나로 떼어내고 */
    nTGcode1 = 0x8000 + (n1st << 10) + (FILL_MID << 5) + FILL_LST;
    nTGcode2 = 0x8000 + (FILL_1ST << 10) + (n2nd << 5) + FILL_LST;
    *pnKScode1 = TG2KS(nTGcode1, FIRST_CODE);
    *pnKScode2 = TG2KS(nTGcode2, MID_CODE);
    return;
  }
}
